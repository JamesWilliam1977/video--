    ...
    def move_temp_paths_to_project_folder(self, file_path, previous_path=None):
        ...
        try:
            ...
            if os.path.abspath(info.THUMBNAIL_PATH) != os.path.abspath(target_thumb_path):
                for root, _, files in os.walk(info.THUMBNAIL_PATH):
                    rel_root = os.path.relpath(root, info.THUMBNAIL_PATH)
                    destination_root = target_thumb_path if rel_root == "." else os.path.join(target_thumb_path, rel_root)
                    if not os.path.exists(destination_root):
                        os.makedirs(destination_root, exist_ok=True)
                    for thumb_path in files:
                        working_thumb_path = os.path.join(root, thumb_path)
                        target_thumb_filepath = os.path.join(destination_root, thumb_path)
            ...
            # Copy all proxy files (if not found in target asset folder)
            if os.path.exists(info.PROXY_PATH) and (
                ...
                for proxy_name in os.listdir(info.PROXY_PATH):
                    ...
                    if os.path.isdir(working_proxy_path):
                        if not os.path.exists(target_proxy_filepath):
                            shutil.copytree(working_proxy_path, target_proxy_filepath)
                    elif not os.path.exists(target_proxy_filepath):
